    ...
    return conn.sendMessage(
      jid,
      {
        ...
        externalAdReply: {
          ...
          thumbnailUrl: ppUrl,
          sourceUrl: "https://whatsapp.com/channel/0029VaAQKcJEquiQVH2RM10U",
          mediaType: 1,
...
exports.smsg = (conn, m, hasParent) => {
  ...
  if (m.key) {
    m.id = m.key.id;
    m.isBaileys =
      (m.id && m.id.length === 16 || m.id.length === 22) ||
  ...
  if (m.message) {
    ...
    if (m.quoted) {
      ...
      m.quoted.isBaileys = (m.quoted.id && m.quoted.id.length === 16 || m.quoted.id.length === 22) || false;
 ...
 const q = {
    ...
    "message": {
      "conversation": "_© Shinaya-Bot Multi device_\n*_TERVERIFIKASI OLEH WHATSAPP ✅_*"
    }
  };
const a = {
        contextInfo: {
            ...
            forwardedNewsletterMessageInfo: {
                newsletterJid: '120363248108328436@newsletter',
                newsletterName: "✅ KLIK BUTTON HERE",
                serverMessageId: -1
            },
            businessMessageForwardInfo: {
                businessOwnerJid: '6283196081684@s.whatsapp.net'
            },
            forwardingScore: 256,            
    externalAdReply: {
        title: `• Nama : ${m.name}\n• Bio: ${bio}`,
        body: `"` + global.Func.random(global.motivasi) + `"`,
        thumbnailUrl: ppUrl,
        sourceUrl: "wa.me/" + m.sender.split("@")[0],
        mediaType: 1,
        renderLargerThumbnail: false
          }
        }
 ...
 try {
 if (options && pesan) {
   return await conn.sendFile(m.chat, options, null, pesan, q, null, a);
  } else {
   if (pesan) {
    return await conn.reply(m.chat, pesan, q, a);
   } else {
    return await conn.reply(m.chat, options, q, a);
   }
  }
 } catch (e) {
  return await conn.reply(m.chat, pesan, q, a);
  ...
  m.reply = async (pesan, options) => {
    ...
    global.fkontak = {
      ...
      message: {
        conversation: "© *ALUXI Multi Device*",
      },
    };
    const a = {
      contextInfo: {
