...
handler.before = async function (m) {
    ...
    if (m.quoted.id == this.math[id][0].id) {
        ...
        if (m.text == math.result) {
           ...
           /* conn.sendFile(m.chat, global.thumb, 'jawaban_benar.jpg', 'Selamat, jawaban kamu benar!', m)*/
        } else {
            if (--this.math[id][2] == 0) {
                clearTimeout(this.math[id][3])
                delete this.math[id]
          ...
          /*conn.sendFile(m.chat, global.thumb, 'jawaban_salah.jpg', 'Sayang sekali, jawaban kamu salah!', m)*/
        }
    }
    return !0
}
