...
let handler = async (m, { conn, text, usedPrefix, command }) => {
   let sdf = `Masukan nama untuk mengecek khodam anda`
   if (!text) return m.reply(sdf);   
          const result = await cekkhodam();
//KONTOL DARI ALOK - WM BINARY
//RECORDER AUDIO BY KRIZZ
          let anu = `[ CEK KHODAM ]\n\nNama : ${text}\nKhodam : ${result.khodam}\nPenjelasan : Penjelasan tentang khodam ${text} adalah ${result.description}`
          m.reply(anu);
          let id = 'id_001'
...
handler.help = ["cekkodam","cekkhodam","ck"].map(a => a + " *[random khodam]*")
handler.tags = ["fun"]
handler.command = ["cekkodam","cekodam","cekkhodam","ck"]
