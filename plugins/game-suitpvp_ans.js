...
klik wa.me/${conn.user.jid.split`@`[0]}`, m, { contextInfo: { mentionedJid: [room.p, room.p2] } })
      ...
      room.waktu_milih = setTimeout(() => {
        if (!room.pilih && !room.pilih2) return this.reply(m.chat, `Kedua pemain tidak niat main,\nSuit dibatalkan`, m)
          ...
          this.reply(m.chat, `@${(room.pilih ? room.p2 : room.p).split`@`[0]} tidak memilih suit, game berakhir`, m, { contextInfo: { mentionedJid: [(room.pilih ? room.p2 : room.p)] } })
...
`.trim(), null, { contextInfo: { mentionedJid: [room.p, room.p2] } })
      if (!tie) {
        db.data.users[win == room.p ? room.p : room.p2].exp += room.poin
        db.data.users[win == room.p ? room.p2 : room.p].exp += room.poin_lose
